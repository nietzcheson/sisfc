...
function Calendar(element, instanceOptions) {
	...
	function calcSize() { // assumes elementVisible
		if (options.contentHeight) {
			...
			suggestedViewHeight = options.height - (headerElement ? headerElement.height() : 0) - vsides(content) -50;
		}
		else {
			suggestedViewHeight = Math.round(content.width() / Math.max(options.aspectRatio, .5)) -50;
		}
	}
	...
	function setSize() { // assumes elementVisible
		...
		elementOuterWidth = element.outerWidth()-100;
	}
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		slotScroller =
			$("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto' id='ht_c_view_2'/>")
				.appendTo(slotLayer);
				
		slotContainer =
			$("<div style='position:relative;width:100%;overflow:hidden' id='ht_c_view_4'/>")
				.appendTo(slotScroller);
				
		slotSegmentContainer =
			$("<div class='fc-event-container' style='position:absolute;z-index:8;top:0;left:0' id='ht_c_view_5'/>")
				.appendTo(slotContainer);
		
		s =
			"<table class='fc-agenda-slots' style='width:100%' cellspacing='0' id='ht_c_view_3'>" +
			"<tbody>";
		...
		while (slotTime < maxTime) {
			...
			s +=
				...
				"</th>" +
				"<td class='" + contentClass + "' style='width:50%;'>" +
				"<div style='position:relative'>&nbsp;</div>" +
				"</td>" +
				"<td class='" + contentClass + "' style='width:50%;background: #f2f2f2;' "+(slotCnt==0 ? 'rowspan="48"' : '')+">" +
				// "<td class='" + contentClass + " sortable' style='width:50%;background: #f2f2f2;'>" +
				// (!minutes ? '' : '<div class="ui-state-default selected-tareas"><p>Tarea # ' + slotCnt + '</p>') +
				// "</div>" +
				"<div style='position:relative' id='contenedor'></div>" +
				"</td>"
				"</tr>";
	...
	function buildDayTableHTML() {
		var html =
			"<table style='width:100%' class='fc-agenda-days fc-border-separate' cellspacing='0' id='ht_c_view'>" +
	...
	function slotClick(ev) {
		if (!opt('selectable')) { // if selectable, SelectionManager will worry about dayClick
			var col = Math.min(colCnt-1, Math.floor((ev.pageX - dayTable.offset().left - axisWidth) / colWidth)-20);
...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			seg.top = top;
			seg.left = left;
			seg.outerWidth = width+width/25-10;
...
function HorizontalPositionCache(getElement) {
	...
	t.right = function(i) {
		return (rights[i] = (rights[i] === undefined ? t.left(i) + e(i).width()/2 : rights[i]));
	};
