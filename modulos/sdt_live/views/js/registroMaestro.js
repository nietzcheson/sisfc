...
$(document).ready(function(){
	...
	$("#submit_email").click(function(){
		...
		$.post(server+'/sdt_live/ennviarEmailRM','dia='+$("#rm_searchdate").val()+'&correo='+$("#correo").val()+'&asunto='+$("#asunto").val(), function(datos){
			if (datos!="None") {
	...
	//offsetT = $("#sample tbody tr:eq(0)").offset();
	...
	$('#registro_maestro').on('change', "#Setiqueta", function(e){
		if (lastObj && SOnline) {
			...
			if (valore!="0") {
				$.post(server+'/sdt_live/getEtiquet','eti='+valore, function(datos){
					//alert(datos);
					if (datos!="None") {
		...
		if (lastObj) {
	        ...
	        if (nu!="1") {
			    ...
			    if (id) {
				    ...
				    $.post(server+'/sdt_live/eliminarRMLinea','id='+id).fail(function() {
					    console.log("error de conexion");
					    SOnline=false;
					});
			    };
    ...
    $("#rm_searchdate").change(function() {
		...
		$.post(server+'/sdt_live/ejemplo1', 'fecha='+$(this).val(),function(datos){
	...
	$('#rm_searchRM').on('click', '.search-day-rm', function(){
		...
		$.post(server+'/sdt_live/ejemplo1', 'fecha='+fecha[2],function(datos){
			$("#rm_contenido").html(datos);
			inicioRM();
...
var LineRM = function(elemento,nume){
	if (SOnline) {
		...
		if ($(trp).attr("id")) {
			id = $(trp).attr("id");
	        id = id.slice(3,id.length);
			$.post(server+'/sdt_live/actualizarLineaTexto','id='+id+'&numeracion='+numeracion+'&orden='+orden+'&texto='+texto+'&tachado='+tachado+'&etiqueta='+inteme).fail(function() {
			    $("#save").removeAttr("disabled");

			});
		}else{
			$.post(server+'/sdt_live/crearLinea','fecha='+fecha+'&numeracion='+numeracion+'&orden='+orden+'&texto='+texto+'&tachado='+tachado +'&etiqueta='+inteme, function(datos){
		        if (datos!="None" && datos!="") {
...
var showDays = function(){
	...
	var fecha = "/"+mes+"/"+anio;
	if (fechaG!=fecha) {
		$.post(server+'/sdt_live/getDias','fecha='+fecha, function(datos){
			datos = datos.split("+");
...
var showTools = function(obj){
 ...
 var hiddeTools = function(){
 	$("#rm_eliminar").css("visibility", "hidden");
