...
$(document).ready(function(){
	...
	$('#ch_contenido').on('click', '.circle', function(){
		...
		if (cambiar) {
			...
			$.post(server+'/sdt/actualizarTareaEstado','dia='+$(div2).attr("id")+'&estado='+estado+"&fecha="+NewDate(dia-4), function(datos){
	        	var indiceHTD = buscarIndicePorIdDia($(div2).attr("id"),NewDate(dia-4));
	        	
	        	if (indiceHTD!=null) {
	        		tareas[NewDate(dia-4)][indiceHTD].estado = estado;
		        	reiniciarEspacio();
					renderizarDatosViejos(NewDate(dia-4));
	        	};
	        	...
	        	if (datos!="None" && datos!="") {
		        	$(div2).attr("id",datos);
		        	if (indiceHTD) {
		        		tareas[NewDate(dia-4)][indiceHTD].id_dia=datos;
		        	}
		        };
	...
	$('#ch_contenido').on('click', '#ch_btn_hoy', function(){
		$.post(server+'/sdt_live/ejemplo3', 'fecha='+$("#rm_searchdate").val()+'&rango='+$("#rangoVista").val()+'&id_usuario='+usuario, function(datos){
	...
	$('#ch_contenido').on('click', '#ch_btn_left', function(){
		...
		$.post(server+'/sdt_live/ejemplo3', 'fecha='+operarFecha(fecha[2],-1)+'&rango='+$("#rangoVista").val()+'&id_usuario='+$("#other_user").val(), function(datos){
			$("#ch_contenido").html(datos);
			inicioCheckList();
			inicioFiltroCheckList();
 		}).fail(function() {
	...
	$('#ch_contenido').on('click', '#ch_btn_right', function(){
		var clase =  $(this).attr("class");
		var fecha = clase.split(" ");
		$.post(server+'/sdt_live/ejemplo3', 'fecha='+fecha[2]+'&rango='+$("#rangoVista").val()+'&id_usuario='+$("#other_user").val(), function(datos){
			$("#ch_contenido").html(datos);
			inicioCheckList();
			inicioFiltroCheckList();
 		}).fail(function() {
	...
	$('#ch_contenido').on('click', '#btn_see_users', function(){
		$('#myModalUsers').modal('show');
	});
	
	$("#other_user").change(function(){
		var clase =  $("#ch_btn_left").attr("class");
		var fecha = clase.split(" ");
		$.post(server+'/sdt_live/ejemplo3', 'fecha='+fecha[2]+'&rango='+$("#rangoVista").val()+'&id_usuario='+$("#other_user").val(), function(datos){
			$("#ch_contenido").html(datos);
			inicioCheckList();
	...
	if (tr.attr("class")!="input-group") {
		...
		if (tr.find(".redondo").length) {
			...
			tr.find(".orange").each(function (index, element) {
				...
				if (clase.indexOf("transferido")===-1) {
					cn+=1;
					var hijo2 = $(element).find(".circle");
					if (clase.indexOf("chuleado")>-1) {
						cn2+=1;
					};
					if (cn2<0) {
						cn2=0;
