<?php 
	class sdt_liveController extends Controller{
		private $_sdt;
		private $_time;
		private $_dateUtils;
		public function __construct(){
			parent::__construct();
			$this->_sdt = $this->loadModel('sdt_live');
		...
		public function index(){
			...
			$this->_view->setJs(array('jquery.drop_down-plugin','ejemplo4'));
			$this->_view->setCss(array('jquery.drop_down-plugin'));

			$this->_view->setJs(array('basico',
				'redips-drag-min',
				'redips-table-min',
				'filtrarCheckList',
				'jCProgress-1.0.3',
				...
				'gcal',
				'hojaTrabajoDiario',
				'jquery.drop_down-plugin'));
			$this->_view->setCss(array('jCProgress',
				'checklist',
				'fullcalendar',
				'jquery.drop_down-plugin'));

			$fechahoy = $fechahoy = date('d/m/Y', mktime()+$this->_time);
			$this->_view->assign('fecha',$fechahoy);
			$this->_view->assign('root', BASE_URL);
			$usurios_sistema=$this->_sdt->getUsuarios2();
			$this->_view->assign('usuariosSistema',$usurios_sistema);
			$usuarios_ele=array();
			foreach ($usurios_sistema as $key) {
				if (trim($key["nombre"])!="") {
					$usuarios_ele[]=array(
						"valor"=>$key["id"],
			...
			$this->_view->assign('usuarios_ele',json_encode($usuarios_ele));
			$seguimiento_ele=array();
			$seguimiento_ele[]=array(
				"valor"=>1,
			...
			$seguimiento_ele[]=array(
				"valor"=>2,
				"clase"=>"glyphicon glyphicon-arrow-right",
				"texto"=>""
			);
			$this->_view->assign('seguimiento_ele',json_encode($seguimiento_ele));

			$prioridad_ele=array();
			$prioridad_ele[]=array(
				"valor"=>1,
				"clase"=>"glyphicon glyphicon-exclamation-sign rojo",
				"texto"=>"Alta"
			);
			$prioridad_ele[]=array(
				"valor"=>2,
				"clase"=>"glyphicon glyphicon-exclamation-sign amarillo",
				"texto"=>"Media"
			);
			$prioridad_ele[]=array(
				"valor"=>3,
				"clase"=>"glyphicon glyphicon-exclamation-sign verde",
				"texto"=>"Baja"
			);
			$prioridad_ele[]=array(
				"valor"=>4,
				"clase"=>"glyphicon glyphicon-exclamation-sign azul",
				"texto"=>"Informativa"
			);
			$this->_view->assign('prioridad_ele',json_encode($prioridad_ele));
			
			$this->_view->assign('id_usuario', Session::get('id_usuario'));
			$usuario = $this->_sdt->getUsuarioSisfcId2(Session::get('id_usuario'));
			$this->_view->assign('nombre_usuario', $usuario["nombre"]);
			$btnHeader = array(
				array(
					"titulo" => "Sincronizar",
					"enlace" => "sdt_live/sincronizar"
				)
			);
			$this->_view->assign("btnHeader",$btnHeader);
			$this->_view->assign("calendarios_publicos",$this->getCalendarios_Publicos());
			
			$this->_view->renderizar('index');
		}
		public function ejemplo1()
		{
			$fecha_post = $this->getTexto("fecha");
			$fecha_post = explode("/", $fecha_post);
			$dia=$fecha_post[0];
			$mes=$fecha_post[1];
			$anio=$fecha_post[2];
			$dife="";

			$fechahoy = date('d/m/Y', mktime()+$this->_time); // "31/08/2014"; //
			...
			$this->_view->renderizarBasico('ejemplo1');
		...
		public function ejemplo3()
		{
			...
			$rango= $this->getSql("rango");
			$id_usuario=$this->getSql("id_usuario");
			...
			for ($i=0; $i < count($datos) ; $i++) {
				$usuario = $this->_sdt->getUsuarioSisfcId2($datos[$i]["id_responsable"]);
			...
			$this->_view->assign('usuariosSistema',$this->_sdt->getUsuarios2());
			$this->_view->assign('base_url',BASE_URL);

			$this->_view->renderizarBasico('ejemplo3');
		}
		public function rm($dia=false,$mes=false,$anio=false,$dife=""){
			...
			echo print_r($this->_sdt->getSDT_RM_Lines(Session::get('id_usuario'),$fecha)); 
			$this->_view->assign('datos',$this->_sdt->getSDT_RM_Lines(Session::get('id_usuario'),"".$fecha.""));
			$etiquetas = $this->_sdt->getSDTFormat_Etiquetas(Session::get('id_usuario'));
			$this->_view->assign('etiquetas',$etiquetas);
		...
		private function diasCiclos($ntoday, $fecha_ini,$fecha_fin,$dias_fijos,$cade_tarea){
			...
			for ($i=0; $i <  count($dias_fijos) ; $i++) {
				if (trim($dias_fijos[$i]["fecha"])!="") {
					...
					// if ($dias_fijos[$i]["pos_htd"]==0 && $fecha_ini == $dias_fijos[$i]["fecha"] ) {
					// 	$pos_htd+=1;
					// 	$dias_fijos[$i]["pos_htd"]=$pos_htd;
					// 	$datos = array(
					// 		"id_dia"=>$dias_fijos[$i]["id_dia"],
					// 		"pos_htd"=>$pos_htd
					// 	);
					// 	$this->_sdt->actualizarSDT_CL_Task_Dia($datos);
					// }
				}
			}
				...
				for ($i=0; $i < count($repetir) ; $i++) { 
					...
					if ($repetir[$i]["opcion"]=="primes") {
						...
						while ($estar) {
							...
							for ($j=0; $j < $repetir[$i]["t_repetir"]; $j++) { 
						...
						$cont=0;
						while ($estar) {
							if ($this->_dateUtils->restarFechas($fecha_ini, $fecha)>0) {
								$estar=false;
							}else{
								for ($j=0; $j < $repetir[$i]["t_repetir"]; $j++) { 
									$fecha =$this->incrementarMes($fecha);
						...
						while ($estar) {
							...
							if ($num<0 || $num2<0)	 {
								...
								// echo $this->_dateUtils->restarFechas($repetir[$i]["fecha_ini"], $fecha2). "<br>";
						...
						$numveces=0;
						$fechaNCiclos="";
						if ($repetir[$i]["t_repetir"]!=0) {
							$numveces+=$repetir[$i]["n_repetir"];
							$fechaNCiclos = $this->_dateUtils->operarAFecha($repetir[$i]["fecha_ini"], 7*($repetir[$i]["t_repetir"]-2),"dias",5000);
							//echo  7*($repetir[$i]["t_repetir"]-2) . "  " . $fechaNCiclos . "<br>";
						}

						while ($estar) {
							// Obtengo la siguientes fechas
							// echo $numciclos."*".$repetir[$i]["repetir"] . "<br>";
							...
							$num3=1;
							if ($fechaNCiclos!="") {
								$num3 = $this->_dateUtils->restarFechas($fecha, $fechaNCiclos);
							}
		...
		public function crearProyecto(){
			...
			if ($this->getInt('crear')=="1")
			{
				...
				if ($errores!="") {
					...
					$this->redireccionar("sdt_live/verProyecto/".$ultimo["id_proyecto"]);
					exit();
				}
			}
		...
		public function verProyecto($proyecto=false){
			$this->_acl->acceso('todo');
			if (!$proyecto) {
				$this->redireccionar("sdt_live/proyecto");
				exit();
			}
			$id_usuario= Session::get('id_usuario');
		...
		public function verTarea($proyecto=false,$tarea=false){
			...
			if (!$tarea) {
				$this->redireccionar("sdt_live/verProyecto/".$proyecto);
				exit();
			}
		...
		public function crearGrupo(){
			...
			if ($this->getInt('crear')=="1")
			{
				...
				if ($errores!="") {
					...
					$this->redireccionar("sdt_live/verGrupo/".$ultimo["id_grupo"]);
					exit();
				}
			}
		...
		public function verGrupo($id=false){
			$this->_acl->acceso('todo');
			if (!$id) {
				$this->redireccionar("sdt_live/indexGrupo") ;
				exit();
			}
			$id_usuario= Session::get('id_usuario');
			...
			if ($this->getInt('crear')=="1"){
				...
				if ($errores!="") {
					...
					$this->redireccionar("sdt_live/crearTareaModelo/".$id."/".$ultimo["id_tarea"]);
					exit();
				}
			}
			$datos = $this->_sdt->getSDT_Grupo($id);
		...
		public function crearTareaModelo($id_grupo=false,$id_tarea=false) {
			$this->_acl->acceso('todo');
			if (!$id_grupo) {
				$this->redireccionar("sdt_live/check_list") ;
				exit();
			}
			if (!$id_tarea) {
				$this->redireccionar("sdt_live/check_list") ;
							...
							for ($i=0; $i < count($dia); $i++) {
								if (trim($dia[$i])!="") {
									if ($posi[$dia[$i]]>=$posi[$ndia]) {
										...
										$nuevaFeche = $this->_dateUtils->operarAFecha($fechaini,7*$n_repetir-($posi[$ndia]-$posi[$dia[$i]])+0.1);
		...
		public function bajarGrupo($id=false){
			...
			$this->_acl->acceso('todo');
			if (!$id) {
				$this->redireccionar('sdt_live/indexGrupos');
				exit();
			}
		...
		public function trasladarHTDUnidad(){
			...
			if ($id_dia!="" && $fecha_traslado!="") {
				// obtener las posiciones libres en 
		...
		public function ennviarEmailRM(){
			...
			if ($dia!="" && $asunto!="")
			{
				...
				$id_usuario=Session::get('id_usuario');
				$user = $this->_sdt->getUsuarioSisfcId2($id_usuario);
				...
				if (is_array($datos)) {
					...
					$mail->From = "www.sdt.sinergiafc.com/";
					$mail->FromName = $user["nombre"]." RM ".$dia;
					$mail->Subject = $asunto;
					$mail->Body = $html;
			...
			if ($id_tarea!="") {
				...
				if ($id_tarea!=0) {
					...
					if ($tarea["id_director"]==$id_usuario) {
						...
						$resultado["url"]=BASE_URL . "sdt_live/verTarea/".$proyecto["id_proyecto"]."/".$id_tarea;
						...
						if (is_array($grupo)) {
							if ($grupo["id_creador"]==$id_usuario) {
								...
								$resultado["url"]=BASE_URL . "sdt_live/crearTareaModelo/".$grupo["id_grupo"]."/".$grupo["id_modelo"];
							}
						}
					}
		...
		public function crearTareaHTD()
		{
			$fecha=$this->getSql("fecha");
			$tarea=$this->getSql("tarea");
			$pos=$this->getSql("pos");
			if ($fecha!="" && $tarea!="") {
				$id_usuario = Session::get('id_usuario');
				$datosEnviar = array(
				...
				$descripcion="Esta tarea fue creada en HTD " + $fecha;
				$datosEnviar = array(
					"id_tarea"=>$ultimo["id_tarea"],
					"descripcion"=>$descripcion
				);
				...
				$this->_sdt->crearSDT_CL_Task_Dia($datosEnviar);
				$id_n = $this->_sdt->ultimoSDT_CL_Days_Line($ultimo["id_tarea"]);
		...
		{
			$this->_view->setJs(array('cssrefresh','buscarRM','registroMaestro','colpick','holdS','jquery.bpopup.min','basico'));
			$this->_view->setCss(array('colpick','stylePopups','style.min'));

			$this->_view->assign('titulo','Basico');
			$this->_view->assign('fecha','31/08/2014');
			$this->_view->renderizar('basico');
		}
		
		public function ejemplo2()
		{
			$this->_view->renderizarBasico('ejemplo2');
		}
		public function htd_eventos(){
			$id_usuario = Session::get('id_usuario');
			if ($id_usuario!=null) {
				echo json_encode($this->_sdt->getHTD_Eventos($id_usuario));
			}
		}
		public function htd_agregar_eventos(){
			$id_usuario = Session::get('id_usuario');
			if ($id_usuario!=null) {
				$title=$this->getSql("title");
				$start=$this->getSql("start");
				$end=$this->getSql("end");
				$datos = array(
					"id_usuario"=>$id_usuario,
					"title"=>$title,
				...
				$this->_sdt->addHTD_Eventos($datos);
			}
		}
		public function htd_modificar_eventos(){
			$id_usuario = Session::get('id_usuario');
			if ($id_usuario!=null) {
				$id=$this->getSql("id");
				$title=$this->getSql("title");
				...
				$this->_sdt->updateHTD_Eventos($datos);
			}
		}
		public function ejemplo4(){
			$this->_view->setJs(array('jquery.drop_down-plugin','ejemplo4'));
			$this->_view->setCss(array('jquery.drop_down-plugin'));
			$this->_view->assign('titulo','ejemplo4');
			$this->_view->renderizar('ejemplo4');
		}

		public function sincronizar(){
			$this->_view->assign('titulo','ejemplo4');
			$id_usuario = Session::get('id_usuario');
			if ($this->getInt('formnrecalendar')=="1") {
				$newurl=$this->getTexto("newurl");
				$newcoment=$this->getTexto("newcoment");
				$activado=$this->getSql("activado");
				$datos = array(
					"id_usuario"=>$id_usuario,
					"activado"=>$activado,
					"url"=>$newurl,
				...
				$this->_sdt->crearHTD_calendario_publico($datos);
			}
			$datos = $this->_sdt->getHTD_calendario_publico_x_usuario($id_usuario);
			$this->_view->assign("datos",$datos);
			$btnHeader = array(
				array(
					"titulo" => "Regresar",
			...
			$this->_view->assign("btnHeader",$btnHeader);
			$this->_view->renderizar('sincronizar');
		}

		public function getCalendarios_Publicos(){
			$id_usuario = Session::get('id_usuario');
			$urls = array();
			if ($id_usuario!=null) {
				$datos = $this->_sdt->getHTD_calendario_publico_x_usuario_x_activado($id_usuario);
				foreach ($datos as $dato) {
					$urls[]=array(
						"url"=>$dato["url"],
