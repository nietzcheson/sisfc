<?php 
	...
	* 
	*/
	class estatus_ventasModel extends Model
	{
		...
		{
	    	$estatus = $this->_db->query("SELECT * FROM estatus_ventas ORDER BY posicion ASC");
			return $estatus->fetchAll();
	    }

	    public function getEstatusId($id)
	    {
	      	$segmentos = $this->_db->query("SELECT * FROM estatus_ventas WHERE id = '$id' ");
			return $segmentos->fetch();
	    }
	    public function crearEstatus($datosEnviar)
	    {
	    	$this->insertarSQL($datosEnviar,"estatus_ventas");
	    }
	    public function actualizarEstatus($datosEnviar)
	    {
	        $this->actualizarSQL($datosEnviar,"estatus_ventas");
	    }
	    public function eliminarEstatus($id)
	    {
	        $post = $this->_db->query("DELETE FROM estatus_ventas WHERE id = '$id'");
	    }
